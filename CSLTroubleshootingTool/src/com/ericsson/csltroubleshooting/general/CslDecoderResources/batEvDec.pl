...
######################
# Updates:
#		 2.1      Fix		 LTNG/flowfox paths updated, fixed path checks and output dir message
...
sub check_environment {
	...
	print "DEBUG: ltng-decoder at: $ltng_tool\n" if $debug;
...
sub load_ctr {
	...
	my $out_dir_exists = 0;
	if ( ($output_dir_user) && (-e $output_dir_user) ) {
	...
	foreach my $file (@ctr_files) {
	   ...
	   my ($dec_file_tmp, $flow_file_tmp);
		...
		if ($translation) {
		   ...
		   my $flow_args = "cat $dec_file | flowfox -w > $flow_file";
		   my $flow_html = "flowfox -f $dec_file -o $flow_file.html";
...
sub final_message {
	...
	print "##########################################\n";
	my $cwd = getcwd;
	print "# Output files stored in ${output_dir}\n\n";
}
...
=head2 Requirements:
   ...
   LteFlowFox tools in the path (i.e. flowfox, ltng and ltng-decoder)
